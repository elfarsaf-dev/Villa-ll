...
<style>
  ...
  #ai-panel{position:fixed;bottom:80px;right:24px;z-index:9998;width:360px;max-width:calc(100vw - 32px);background:#fff;border-radius:20px;box-shadow:0 8px 48px rgba(0,0,0,.18);display:flex;flex-direction:column;overflow:hidden;transition:opacity .25s,transform .25s,visibility .25s;opacity:0;transform:translateY(14px) scale(.97);pointer-events:none;visibility:hidden;overscroll-behavior:contain}
  ...
  #ai-msgs{flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:9px;height:300px;overscroll-behavior:contain;-webkit-overflow-scrolling:touch}
...
(function(){
  var msgs=document.getElementById('ai-msgs');
  if(!msgs) return;
  msgs.addEventListener('touchstart',function(e){this._startY=e.touches[0].clientY;},{passive:true});
  msgs.addEventListener('touchmove',function(e){
    var dy=e.touches[0].clientY-(this._startY||0);
    var atTop=this.scrollTop===0;
    var atBot=this.scrollTop+this.clientHeight>=this.scrollHeight-1;
